    ...
    const fetchAttendence = async () => {
      try {
        const response = await axios.get(
          `https://attendence-five.vercel.app/api/attendence/${id}`
        );
        const { attendence } = response.data;
        setAttendence(attendence.Attendance);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  return (
    <ScrollView>
      ...
      <View style={{ margin: 10 }}>
        <Text style={{ fontWeight: "500" }}>Viewing Attendence</Text>
        {attendence.map((item) => (
          <List.Item
            title={item.status}
